...
test("Pandits receive repeating loud alarms for Puja and chat requests", async () => {
  ...
  const guidance = await readFile(new URL("../components/pandit-guidance-settings.tsx", import.meta.url), "utf8");
  ...
  assert.match(guidance, /chatRequests=\{urgentChatIds\.length\}/);
