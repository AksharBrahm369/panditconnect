self.addEventListener("push", (event) => {
  ...
  event.waitUntil(Promise.all([
    self.registration.showNotification(data.title, { body: data.body, icon: "/favicon.svg", badge: "/favicon.svg", data: { url: data.url }, tag: data.eventType || "panditconnect-update", renotify: true, silent: false, requireInteraction: data.eventType === "BOOKING_REQUESTED", vibrate: [180, 80, 180] }),
