...
export function NotificationCenter() {
  ...
  async function updatePreference(key:keyof Preferences,value:boolean){const next={...preferences,[key]:value};setPreferences(next);await fetch("/api/notification-preferences",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({bookingUpdates:next.booking_updates,chatUpdates:next.chat_updates,serviceUpdates:next.service_updates,marketing:next.marketing})});}
  return <div className="notification-center"><button className="icon-button notification-button" onClick={() => open ? setOpen(false) : void show()} aria-label="Notifications">{unread ? <BellRing size={18} /> : <Bell size={18} />}{unread > 0 && <b>{Math.min(unread, 9)}{unread > 9 ? "+" : ""}</b>}</button>{open && <section className="notification-panel"><header><div><strong>Notifications</strong><small>Updates for this account</small></div><button className="icon-button" onClick={() => setOpen(false)} aria-label="Close notifications"><X size={16} /></button></header>{!enabled ? <div className="notification-permission"><BellRing size={19} /><div><strong>Never miss an urgent update</strong><small>Receive booking, chat and review alerts with sound on this device.</small></div><button onClick={() => void enable()}>Enable</button></div> : <div className="notification-enabled"><p className="notification-message"><Check size={14} />Device alerts and in-app sound are on.</p><button onClick={() => void testAlert()}>Test sound</button></div>}{message && <p className="notification-message"><Check size={14} />{message}</p>}<div className="notification-preferences"><strong>Alert preferences</strong><label><input type="checkbox" checked={preferences.booking_updates} onChange={e=>void updatePreference("booking_updates",e.target.checked)}/>Booking updates</label><label><input type="checkbox" checked={preferences.chat_updates} onChange={e=>void updatePreference("chat_updates",e.target.checked)}/>Chat updates</label><label><input type="checkbox" checked={preferences.service_updates} onChange={e=>void updatePreference("service_updates",e.target.checked)}/>Account and service updates</label></div><div className="notification-list">{items.length ? items.map((item) => <a href={item.url} key={item.id} className={!item.read_at ? "unread" : ""}><strong>{item.title}</strong><span>{item.body}</span><small>{new Date(item.created_at).toLocaleString("en-IN")}</small></a>) : <p className="notification-empty">No notifications yet.</p>}</div></section>}</div>;
}
