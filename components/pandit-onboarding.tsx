...
type ApiProfile = { name?: string; phone?: string; email?: string; date_of_birth?: string; city?: string; current_address?: string; experience_years?: number; languages?: string[]; specialities?: string[]; bio?: string; service_radius_km?: number; availability_preference?: "AVAILABLE_AFTER_APPROVAL" | "OFFLINE"; payout_method?: "BANK" | "UPI"; bank_account_name?: string; bank_ifsc?: string; upi_id?: string; interview_mode?: "LIVE_VIDEO_CALL" | "RECORDED_VIDEO"; interview_preferred_at?: string; interview_alternate_at?: string; interview_language?: string; interview_note?: string; platform_rules_accepted_at?: string };
...
export function PanditOnboarding({ status, reviewNote, onSaved }: { status: string; reviewNote?: string | null; onSaved: () => void }) {
  ...
  async function load() {
    ...
    if (!dirtyRef.current) {
      ...
      setForm((old) => ({ ...old, legalName: p.name ?? "", phone: p.phone ?? "", email: p.email ?? "", dateOfBirth: p.date_of_birth?.slice(0, 10) ?? "", city: p.city ?? "", currentAddress: p.current_address ?? "", experienceYears: p.experience_years ?? 0, languages: p.languages?.join(", ") || "Hindi", specialities: p.specialities?.join(", ") || "", bio: p.bio ?? "", serviceRadiusKm: p.service_radius_km ?? 10, availabilityPreference: p.availability_preference ?? "OFFLINE", payoutMethod: p.payout_method ?? "UPI", bankAccountName: p.bank_account_name ?? "", bankIfsc: p.bank_ifsc ?? "", upiId: p.upi_id ?? "", interviewMode: p.interview_mode ?? "LIVE_VIDEO_CALL", interviewPreferredAt: p.interview_preferred_at?.slice(0, 16) ?? "", interviewAlternateAt: p.interview_alternate_at?.slice(0, 16) ?? "", interviewLanguage: p.interview_language ?? "Hindi", interviewNote: p.interview_note ?? "", acceptPlatformRules: Boolean(p.platform_rules_accepted_at) }));
    }
    setHydrated(true);
  }
  ...
  const missingRequirements = useMemo(() => {
    ...
    const requirements: Array<[boolean, string]> = [
      ...
      [form.interviewLanguage.trim().length > 0, "Select your preferred interview language"],
      [form.interviewMode === "LIVE_VIDEO_CALL" ? Boolean(form.interviewPreferredAt) : uploaded.has("VIDEO_INTERVIEW"), form.interviewMode === "LIVE_VIDEO_CALL" ? "Select a preferred live video interview time" : "Upload your recorded video introduction"],
  ...
  const progress = Math.round(((17 - missingRequirements.length) / 17) * 100);
  ...
  async function upload(type: string, file?: File) {
    ...
    if (response.ok && result.document) {
      ...
      setNotice(type === "VIDEO_INTERVIEW" ? "Interview video uploaded successfully and saved in Supabase." : "Document stored privately in Supabase.");
  ...
  return <section className="trusted-onboarding" onChangeCapture={() => { dirtyRef.current = true; }}>
    ...
    {["SUBMITTED", "UNDER_REVIEW"].includes(status) && <div className="review-in-progress-banner"><BadgeCheck size={22} /><span><strong>Your application is {status.replaceAll("_", " ").toLowerCase()}.</strong> You can still add interview availability or correct information using Save draft.</span></div>}
    ...
    <div className="onboarding-section interview-section"><h3>5. Video interview</h3><p className="section-help">Choose how you want to complete the short identity and Puja-experience interview.</p><div className="interview-mode-grid"><label className={form.interviewMode === "LIVE_VIDEO_CALL" ? "selected" : ""}><input type="radio" name="interviewMode" checked={form.interviewMode === "LIVE_VIDEO_CALL"} onChange={() => setForm({ ...form, interviewMode: "LIVE_VIDEO_CALL" })} /><span><strong>Live video call</strong><small>Select two suitable times. The admin will confirm one.</small></span></label><label className={form.interviewMode === "RECORDED_VIDEO" ? "selected" : ""}><input type="radio" name="interviewMode" checked={form.interviewMode === "RECORDED_VIDEO"} onChange={() => setForm({ ...form, interviewMode: "RECORDED_VIDEO" })} /><span><strong>Recorded introduction</strong><small>Upload a short video introduction for admin review.</small></span></label></div><div className="form-grid"><label>Interview language *<input value={form.interviewLanguage} onChange={(e) => setForm({ ...form, interviewLanguage: e.target.value })} placeholder="Hindi" /></label>{form.interviewMode === "LIVE_VIDEO_CALL" ? <><label>Preferred time *<input type="datetime-local" value={form.interviewPreferredAt} onChange={(e) => setForm({ ...form, interviewPreferredAt: e.target.value })} /></label><label>Alternative time<input type="datetime-local" value={form.interviewAlternateAt} onChange={(e) => setForm({ ...form, interviewAlternateAt: e.target.value })} /></label></> : <label className={`recorded-interview-upload ${documents.some((doc) => doc.document_type === "VIDEO_INTERVIEW") ? "complete" : ""}`}>Recorded video *<input type="file" accept="video/mp4,video/webm,video/quicktime,.mov" disabled={uploading === "VIDEO_INTERVIEW"} onChange={(e) => void upload("VIDEO_INTERVIEW", e.target.files?.[0])} /><small>{uploading === "VIDEO_INTERVIEW" ? "Uploading securely…" : documents.find((doc) => doc.document_type === "VIDEO_INTERVIEW") ? `${documents.find((doc) => doc.document_type === "VIDEO_INTERVIEW")!.original_name} · SAVED` : "MP4, WebM or MOV up to 50 MB"}</small></label>}<label className="span-2">Message for the interviewer<textarea rows={3} value={form.interviewNote} onChange={(e) => setForm({ ...form, interviewNote: e.target.value })} placeholder="Mention accessibility needs or preferred calling hours." /></label></div></div>
    <label className="rules-consent"><input type="checkbox" checked={form.acceptPlatformRules} onChange={(e) => setForm({ ...form, acceptPlatformRules: e.target.checked })} /><span><strong>I agree to the platform rules and verification process.</strong><small>I confirm that all information is accurate and permit identity, reference, interview, knowledge and payout verification.</small></span></label>
