...
export function PrivacyRightsSettings({role}:{role:"CUSTOMER"|"PANDIT"}){
  ...
  if(deletionSubmitted)return <section className="private-settings-card privacy-deletion-submitted"><span><ShieldCheck/></span><h2>Deletion request sent to Admin</h2><p>Your profile has not been deleted yet. It is hidden from new customer requests while an authorised Admin checks active services and balances. Deletion happens only after Admin approval; then the private profile is removed and the account anonymised.</p><small>You have been securely signed out. Returning to the home page…</small><a className="btn btn-primary" href="/">Go to home page</a></section>;
