...
export function AdminLoginForm() {
  ...
  async function requestOtp() {
    ...
    try {
      ...
      const data = await readJson<{ error?: string; devOtp?: string }>(response);
      if (!response.ok) throw new Error(data.error ?? "Administrator access could not be verified.");
  ...
  return <div className="auth-shell admin-auth-shell">
    ...
    <section className="auth-form-wrap"><div className="auth-card">
      ...
      {step === "phone" ? <>
        ...
        <button className="text-button" onClick={() => { setStep("phone"); setOtp(""); setDevOtp(""); }}>Change mobile number</button>
      </>}
    </div></section>
  </div>;
}
