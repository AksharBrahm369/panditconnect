...
function OutstandingBalanceNotice({ amount, booking, canPayOnline, paying, paymentMessage, onPay }: {
  ...
  const supportHref = `mailto:${customerCareEmail}?subject=${encodeURIComponent("PujaOne outstanding payment support")}&body=${encodeURIComponent(`Hello PujaOne Support,\n\nI need help with my outstanding payment of ₹${amount.toLocaleString("en-IN")}.\n\nThank you.`)}`;
...
export function CustomerPortal({ customerId, customerName, initialStart }: { customerId: string; customerName?: string | null; initialStart?: CustomerStart }) {
  ...
  const [nearbySearchLocation, setNearbySearchLocation] = useState<BrowserCoordinates | null>(null);
  ...
  async function sendRequest(panditId: string, confirmedLocation?: BrowserCoordinates | null) {
    if (requestingPanditId) return;
    const requestLocation = confirmedLocation ?? nearbySearchLocation ?? coordinates;
    if (!requestType) {
      setNearbyRequestError("Your Puja details are no longer available. Tap Edit request and confirm them again.");
      return;
    }
    if (!requestLocation) {
      setNearbyRequestError("Your location needs to be confirmed again before sending this request.");
      return;
    }
  ...
  async function startOnlinePayment(bookingId:string,purpose:"SERVICE_PAYMENT"|"CANCELLATION_FEE"="SERVICE_PAYMENT"){
    ...
    const response=await fetch("/api/payments/orders",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({bookingId,purpose,idempotencyKey})});
    const data=await readJson<{error?:string;transactionId?:string;orderId?:string;amount?:number;keyId?:string;status?:string;checkoutName?:string;prefill?:{name?:string;email?:string;contact?:string}}>(response);
    if(!response.ok||!data.transactionId||!data.orderId||!data.keyId){setPaymentMessages(current=>({...current,[bookingId]:data.error??"Unable to start secure payment."}));setPaymentBusy(null);return;}
    if(data.status==="CAPTURED"){setPaymentMessages(current=>({...current,[bookingId]:"Payment is already confirmed."}));await refreshBookings();setPaymentBusy(null);return;}
    try{await ensureRazorpayCheckout();}catch(error){setPaymentMessages(current=>({...current,[bookingId]:error instanceof Error?error.message:"Secure checkout could not load."}));setPaymentBusy(null);return;}
    if(!window.Razorpay){setPaymentMessages(current=>({...current,[bookingId]:"Secure checkout is unavailable. Please retry."}));setPaymentBusy(null);return;}
    const checkout=new window.Razorpay({key:data.keyId,amount:(data.amount??0)*100,currency:"INR",name:data.checkoutName??"PujaOne",description:purpose==="CANCELLATION_FEE"?"Outstanding cancellation payment":"Completed Puja payment",order_id:data.orderId,prefill:data.prefill,method:purpose==="CANCELLATION_FEE"?{upi:true,card:false,netbanking:false,wallet:false}:{upi:true,card:true,netbanking:false,wallet:false},handler:async(result:RazorpayCheckoutSuccess)=>{setPaymentMessages(current=>({...current,[bookingId]:"Verifying payment securely…"}));const verifiedResponse=await fetch("/api/payments/verify",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({transactionId:data.transactionId,...result})});const verified=await readJson<{success?:boolean;status?:string;error?:string}>(verifiedResponse);if(verifiedResponse.ok&&verified.success){setPaymentMessages(current=>({...current,[bookingId]:"Payment confirmed securely."}));await refreshBookings();}else if(verifiedResponse.ok&&verified.status==="PENDING"){setPaymentMessages(current=>({...current,[bookingId]:"Payment received. Bank confirmation is pending and will update automatically."}));window.setTimeout(()=>void refreshBookings(),2500);}else setPaymentMessages(current=>({...current,[bookingId]:verified.error??"Payment could not be verified. If money was deducted, it will be reconciled automatically."}));setPaymentBusy(null);},modal:{ondismiss:()=>setPaymentBusy(null)},theme:{color:"#c54824"}});
    checkout.on("payment.failed",()=>{setPaymentMessages(current=>({...current,[bookingId]:"Payment failed or was cancelled. No success was recorded."}));setPaymentBusy(null);});checkout.open();
  }
        ...
        <section className="nearby-chooser" id="nearby-pandits">
          ...
          {nearbyPandits.length ? <><p className="nearby-scope-note"><MapPin size={15} /> Matched for your Puja, language and current location.</p>{nearbyRequestError && <><div className="alert error nearby-request-feedback" id="nearby-request-feedback" role="alert"><AlertTriangle size={18} /><span><strong>Request not sent</strong>{nearbyRequestError}</span></div>{nearbyRequestErrorCode === "OUTSTANDING_BALANCE" && outstandingBalance > 0 && <OutstandingBalanceNotice amount={outstandingBalance} booking={outstandingBooking} canPayOnline={onlinePayments} paying={paymentBusy === outstandingBooking?.id} paymentMessage={outstandingBooking ? paymentMessages[outstandingBooking.id] : undefined} onPay={() => outstandingBooking && void startOnlinePayment(outstandingBooking.id, "CANCELLATION_FEE")} />}</>}<div className="nearby-pandit-grid">{sortedNearbyPandits.map((pandit) => (
            <article className="nearby-pandit-card" key={pandit.id}>
              ...
              <button type="button" className="btn btn-primary btn-block nearby-request-button" disabled={requestingPanditId !== null} aria-busy={requestingPanditId === pandit.id} onClick={() => void sendRequest(pandit.id, nearbySearchLocation)}>{requestingPanditId === pandit.id ? "Sending your request…" : requestingPanditId ? "Please wait…" : `Send request to ${pandit.name}`} <ChevronRight size={17} /></button>
            </article>
