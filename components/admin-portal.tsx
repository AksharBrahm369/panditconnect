    ...
    {queueOpen && <div className="review-overlay" role="dialog" aria-modal="true" aria-label="Pandit verification queue">
      <section className="review-drawer">
        ...
        {!selected ? <>
          ...
          <div className="review-section admin-interview-summary"><span>Video interview preference</span><strong>{selected.interview_mode === "RECORDED_VIDEO" ? "Recorded video introduction" : "Live video call"}</strong><p>Language: {selected.interview_language || "Not selected"}</p>{selected.interview_mode !== "RECORDED_VIDEO" && <p>Preferred: {selected.interview_preferred_at ? new Date(selected.interview_preferred_at).toLocaleString("en-IN") : "Not selected"}<br />Alternative: {selected.interview_alternate_at ? new Date(selected.interview_alternate_at).toLocaleString("en-IN") : "Not selected"}</p>}{selected.interview_note && <p>Note: {selected.interview_note}</p>}<small>{selected.interview_mode === "RECORDED_VIDEO" ? "Open the VIDEO INTERVIEW document above before verifying." : "Confirm the final interview time in the checklist below."}</small></div>
