...
export async function GET(request: Request) {
  try {
    ...
    if (approvedOnly) {
      const approved = await sql(`SELECT u.id,u.name,u.phone,u.city,u.created_at,u.account_status,u.account_status_reason,u.account_status_changed_at,p.experience_years,p.languages,p.specialities,p.bio,p.base_charge,p.verification_status,p.review_note,p.is_online,p.rating,p.rating_count,p.completed_jobs,COALESCE(array_agg(DISTINCT s.name) FILTER (WHERE s.name IS NOT NULL),'{}') AS services,count(*) OVER()::int AS total_count FROM pim_v2.pandit_profiles p JOIN pim_v2.users u ON u.id=p.user_id LEFT JOIN pim_v2.pandit_services ps ON ps.pandit_id=p.user_id LEFT JOIN pim_v2.services s ON s.id=ps.service_id WHERE p.verification_status='APPROVED' GROUP BY u.id,u.name,u.phone,u.city,u.created_at,u.account_status,u.account_status_reason,u.account_status_changed_at,p.experience_years,p.languages,p.specialities,p.bio,p.base_charge,p.verification_status,p.review_note,p.is_online,p.rating,p.rating_count,p.completed_jobs ORDER BY CASE u.account_status WHEN 'ACTIVE' THEN 0 WHEN 'RESTRICTED' THEN 1 ELSE 2 END,p.is_online DESC,p.rating DESC,u.name LIMIT $1 OFFSET $2`, [limit, offset]);
